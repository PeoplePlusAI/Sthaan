...
def state_genericadr_type():
    ...
    replay_chat()
    st.text_input(label=USER_AVATAR, key=json_keys[idx], on_change=fetch_genericadr_details, args=(question, idx))

def fetch_genericadr_details(*args):
    question = ''
    idx = None
    if any(args): 
        question = args[0]
